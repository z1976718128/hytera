	...
	$.fn.pbTouchSlider = function(options) {
		...
		function setResponsive() {
			...
			if(VW >= slider_Opts.slider_Breakpoints.tablet.media) {
				$(pbSlider.slider_Wrap + '.o-sliderContainer,' + pbSlider.slider_Wrap + ' ' + pbSlider.slider_Item).css({
					height: slider_Opts.slider_Breakpoints.default.height
				});
			} else if(VW >= slider_Opts.slider_Breakpoints.smartphone.media) {
				$(pbSlider.slider_Wrap + '.o-sliderContainer,' + pbSlider.slider_Wrap + ' ' + pbSlider.slider_Item).css({
					height: slider_Opts.slider_Breakpoints.tablet.height
				});
			} else {
				$(pbSlider.slider_Wrap + '.o-sliderContainer,' + pbSlider.slider_Wrap + ' ' + pbSlider.slider_Item).css({
					height: slider_Opts.slider_Breakpoints.smartphone.height
				});
			}
		}
...
//		$('head').append('<style>' + pbSlider.slider_Wrap + ' .o-slider.isAnimate{' +
...
//			'</style>');
		...
		pbSlider.pbInit = function(selector) {
			...
			$(pbSlider.slider_Item).css({
				'width': 100 / pbSlider.slider_Count + '%'
			});
			var incrSlides = 0;
			...
			if(slider_Opts.slider_Dots.enabled === true) {
				...
				for(incrPagination; incrPagination < pbSlider.slider_Count; incrPagination++) {
					...
					if(slider_Opts.slider_Dots.preview === true) {
						$(pbSlider.slider_Wrap).find(pbSlider.slider_Dots.class).append('<li ' + activeStatus + ' ' + gotoSlide + '>' +
							'<span class="o-slider--preview" style="background-image:url(' + background + ')"></span>' +
							'</li>');
					} else {
						$(pbSlider.slider_Wrap).find(pbSlider.slider_Dots.class).append('<li ' + activeStatus + ' ' + gotoSlide + '></li>');
					}
				}
			}
			setTimeout(function() {
